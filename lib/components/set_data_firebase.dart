...
class SetDataFirebase{
  ...
  static Future<void> initData(BuildContext context) async {
    ...
    for (var cityName in cities) {
      ...
      for (var cinemaGalaxy in cinemaCity.galaxy!) {
        await setMovieForCinema(cinemaGalaxy, listNowShowing, cityName, "Galaxy");
      }

      for (var cinemaLotte in cinemaCity.lotte!) {
        await setMovieForCinema(cinemaLotte, listNowShowing, cityName, "Lotte");
      }
      
    }
  }

  static Future<void> setMovieForCinema(Cinema cinemas,List<Movie> listNowShowing,  String cityName, String cinemaStyle)async{
    int i = 0;
    ...
    while(i < 14){
      for (var movie in listNowShowing) {
        ...
        await FirebaseFirestore.instance.collection("Cinemas").doc(cityName).collection(cinemaStyle).doc(cinemas.id).collection("$day-$month-$year").doc(movie.id).set(
          {
            "movie": {
              "id": movie.id,
              "name": movie.name,
            ...
            "subtitle_2D": [
              {"time": addDurationToTime("10:0$i", movie.duration!) , "roomID": cinemas.rooms![roomRandom1].id},
        ...
        await setInitSeats(
          colum: cinemas.rooms![roomRandom1].column!, row: cinemas.rooms![roomRandom1].row!, showtimes: "${addDurationToTime("10:0$i", movie.duration!)} - ${cinemas.rooms![roomRandom1].id}", 
          cityNam: cityName, cinemaID: cinemas.id, date: "$day-$month-$year", movieID: movie.id!, cinemaType: cinemaStyle
        );

        await setInitSeats(
          colum: cinemas.rooms![roomRandom2].column!, row: cinemas.rooms![roomRandom2].row!, showtimes: "${addDurationToTime("14:00", movie.duration!)} - ${cinemas.rooms![roomRandom2].id}", 
          cityNam: cityName, cinemaID: cinemas.id, date: "$day-$month-$year", movieID: movie.id!, cinemaType: cinemaStyle
        );

        await setInitSeats(
          colum: cinemas.rooms![roomRandom3].column!, row: cinemas.rooms![roomRandom3].row!, showtimes: "${addDurationToTime("16:00", movie.duration!)} - ${cinemas.rooms![roomRandom3].id}", 
          cityNam: cityName, cinemaID: cinemas.id, date: "$day-$month-$year", movieID: movie.id!, cinemaType: cinemaStyle
        );
  ...
  static setInitSeats(
    {required int colum, required int row, required String showtimes, required String cityNam, required String cinemaID, required String date, required String movieID, 
  ...
  static Future<CinemaCity> getCinemaCity(String cityName) async {
    final reponse = await FirebaseFirestore.instance.collection("Cinemas").doc(cityName).get();
...
String addDurationToTime(String startTime, int minutesToAdd) {
    ...
    DateTime endTimeDateTime = startTimeDateTime.add(Duration(minutes: minutesToAdd));
    
    String endTime = "${endTimeDateTime.hour.toString().padLeft(2, '0')}:${endTimeDateTime.minute.toString().padLeft(2, '0')}";
