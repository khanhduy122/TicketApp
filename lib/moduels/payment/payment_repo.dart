...
class PaymentRepo {
  ...
  Future<void> addMethodPayment(PaymentCard card) async {
    try {
      await _firestore.collection("User").doc(FirebaseAuth.instance.currentUser!.uid).collection("Payments").doc(card.cardNumber).set(card.toJson());
    } catch (e) {
      rethrow;
    }
  }
  ...
  Future<List<PaymentCard>> getMethodPaymentUser() async {
    try {
      ...
      final response = await _firestore.collection("User").doc(FirebaseAuth.instance.currentUser!.uid).collection("Payments").get();
