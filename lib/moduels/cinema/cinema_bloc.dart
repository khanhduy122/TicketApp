...
class CinemaBloc extends Bloc<CinemaEvent, CinemaState> {
  ...
  CinemaBloc() : super(CinemaState()){
    ...
    on<GetAllMovieReleasedCinemaEvent>((event, emit) => _getAllMovieReleasedCinema(event, emit));

    on<GetCinemasShowingMovieEvent>((event, emit) => _getCinemasShowingMovie(event, emit));
  }
  ...
  void _getCinemaCity(GetCinemaCityEvent event, Emitter emit) async {
    ...
    try {
      ...
      final response = await _cinemasRepo.getCinemasCity(event.cityName, event.context);
  ...
  void _getAllMovieReleasedCinema(GetAllMovieReleasedCinemaEvent event, Emitter emit) async {
    ...
    try {
      ...
      final response = await _cinemasRepo.getAllMovieReleasedCinema(cityName: event.cinema.cityName, date: event.date, cinema: event.cinema);
  ...
  void _getCinemasShowingMovie(GetCinemasShowingMovieEvent event, Emitter emit) async {
    ...
    try {
      ...
      final response = await _cinemasRepo.getAllCinemasShowingMovie(city: event.cityName, movieID: event.movieID, date: event.date, context: event.context);
