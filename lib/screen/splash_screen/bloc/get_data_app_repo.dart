...
class GetDataAppRepo {
  ...
  Future<HomeData> getMovieApp() async {
    ...
    return HomeData(banners: homeBanner, nowShowing: homeNowShowing, comingSoon: homeComingSoon);
  }

  Future<List<Review>> getReviewMovie({required String movieID}) async {
    List<Review> reviews = [];
    final res = await _firestore.collection("Now Showing").doc(movieID).collection("reviews").orderBy("timestamp", descending: true).limit(10).get();
  ...
  Future<CinemaCity> getCinemasRecommended() async {
    ...
    String city = await getCurrentCity(position.latitude, position.longitude); 
    final res = await _firestore.collection("data_app").doc("Sóc Trăng").get();
