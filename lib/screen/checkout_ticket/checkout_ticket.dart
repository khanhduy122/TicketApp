...
class _CheckoutTicketScreenState extends State<CheckoutTicketScreen> with WidgetsBindingObserver {

  final PaymentBloc paymentBloc = PaymentBloc();
  String id = (Random().nextInt(99999999) + 100000000).toString() + DateFormat('yyyyMMddHHmmss').format(DateTime.now()).toString();
  Timer? timmer;
          ...
          body: Padding(
            ...
            child: Column(
              children: [
                ...
                _buidHeader(),
                Divider(height: 2.h, color: AppColors.white,),
                SizedBox(height: 20.h,),
    ...
    return SizedBox(
      ...
      child: Column(
        children: [
          Row(
            children: [
              Text("Thời gian giữ ghế", style: AppStyle.defaultStyle,),
              SizedBox(width: 10.w,),
              StreamBuilder(
                ...
                builder: (context, snapshot) {
                  return Container(
                    ...
                    child: Text(formatTimeCountDown(currentSecond), style: AppStyle.defaultStyle.copyWith(color: AppColors.white),)
                  );
                }
              ),
            ],
    ...
    return Expanded(
      child: Column(
        ...
        children: [
          ...
          _buildItemInformation(title: "Rạp: ", value: widget.ticket.cinema!.name),
          _buildItemInformation(title: "Ngày giờ: ", value: formatDateTime(showtimes: widget.ticket.showtimes!, dateTime: widget.ticket.date!)),
          _buildItemInformation(title: "Ghế: ", value: formatListSeat()),
          _buildItemInformation(title: "Tổng tiền: ", value: "${formatPrice(widget.ticket.price!)} VND"),
        ],
      ),
    );
  }
  ...
  Widget _buildItemInformation({required String title, required String value}){
    return Row(
      ...
      children: [
        ...
        Expanded(child: Text(value, style: AppStyle.defaultStyle.copyWith(color: AppColors.white), maxLines: 19, softWrap: true, textAlign: TextAlign.end,)),
      ],
    );
  }

  String formatDateTime({required String showtimes, required DateTime dateTime}){
  ...
  void _onListener(Object? state){
    if(state is GetMethodPaymentUserState){
      ...
      if(state.listCard != null){
        widget.ticket.id = id; 
        Navigator.pop(context);
        Navigator.pushNamed(context, RouteName.selectCardScreen, arguments: {"listCard" :state.listCard, "ticket": widget.ticket});
      }
