...
class _SelectCardScreenState extends State<SelectCardScreen> {
  ...
  final StreamController controllerDisplayWebview =
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                    ...
                    builder: (context, state) {
                      if (state is GetMethodPaymentUserState) {
                        ...
                        if (state.listCard != null) {
                          listCardUser = state.listCard!.sublist(0);
                ...
                _buildButtonAddCard(onTap: _onTapCreateMethodPaymemt),
                SizedBox(
                  height: 20.h,
                ),
                widget.ticket != null
                ...
                return Column(
                  children: [
                    _buildItemCard(
                      card: listCard[index],
                      isSelected:
                          widget.ticket != null ? currentIndex == index : false,
                      onTap: () {
                        setState(() {
  ...
  Widget _buildItemCard(
    ...
    return GestureDetector(
      onTap: onTap,
      onLongPress: widget.ticket == null
          ? () async {
              final isConfirm = await DialogConfirm.show(
                  context: context,
                  message: "Bạn có chắc muốn xóa liên kết thẻ thanh toán này");
              if (isConfirm) {
                paymentBloc.add(DeleteMethodPaymentEvent(card: card));
              }
            }
          : null,
      child: Container(
  ...
  void _handleErrorPayment(String errorCode) {
    switch (errorCode) {
      case "02":
        DialogError.show(
            context: context, message: "Giao dịch lỗi, vui lòng thử lại");
        break;
      case "04":
        DialogError.show(
            context: context,
  ...
  Future<void> _listener(Object? state) async {
    ...
    if (state is AddTicketState) {
      ...
      if (state.isSuccess == true) {
        ...
        Navigator.pushNamed(context, RouteName.paymentSuccessScreen,
    ...
    if (state is DeleteMethodPaymentState) {
      if (state.isLoading == true) {
        DialogLoading.show(context);
      }
  ...
  void _onTapPayment() async {
    await Navigator.pushNamed(context, RouteName.paymentScreen, arguments: {
      "ticket": widget.ticket,
      "paymentCard": listCardUser[currentIndex!]
    }).then((response) {
